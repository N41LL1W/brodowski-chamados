...
export async function POST(req: Request) {
    ...
    if ((session?.user as any)?.role !== 'MASTER') return new NextResponse('Unauthorized', { status: 403 });
    ...
    if (type === 'level') {
        const newLevel = await prisma.level.create({ data: { name, rank: parseInt(rank) || 1 } });
