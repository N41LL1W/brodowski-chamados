...
import { 
  ...
  Clock, Navigation, Filter, Terminal
} from 'lucide-react';
...
export default function DetalheChamadoPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    if (loading) return (
        <div className="h-screen flex flex-col items-center justify-center bg-background dark:bg-slate-950">
            ...
            <p className="text-slate-400 dark:text-muted font-black uppercase text-xs">Sincronizando...</p>
        </div>
    );
    ...
    return (
        <div className="min-h-screen bg-background dark:bg-slate-950 transition-colors duration-300 pb-12">
            ...
            {/* HEADER STICKY */}
            <div className="bg-card dark:bg-slate-900 border-b border-border dark:border-slate-800 sticky top-0 z-30 px-4 py-3">
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-sm font-black text-slate-800 dark:text-slate-100 uppercase truncate max-w-[200px]">{ticket.subject}</h1>
                    ...
                    <Card className="p-6 rounded-3xl border-none shadow-sm bg-card dark:bg-slate-900">
                        <div className="space-y-4">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(ticket.location)}`} target="_blank" className="p-2.5 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 rounded-xl">
                                    <Navigation size={18} />
                                </a>
                            </div>
                        </div>
                    ...
                    <Card className="flex-1 flex flex-col p-0 rounded-3xl border-none shadow-sm bg-card dark:bg-slate-900 overflow-hidden">
                            ...
                            {ticket?.comments?.filter((c: any) => {
                                ...
                                return (
                                    <div key={c.id} className={`flex flex-col ${isInternal ? 'items-center' : (isTecnico ? 'items-end' : 'items-start')}`}>
                                        <span className="text-[9px] font-bold text-slate-400 dark:text-muted mb-1 px-2">
                                        ...
                                        <div className={`p-4 rounded-2xl text-sm max-w-[85%] ${
                                            ...
                                            isTecnico ? 'bg-slate-900 dark:bg-blue-600 text-white shadow-sm' : 'bg-slate-100 dark:bg-slate-800 text-slate-800 dark:text-slate-200'
                                        }`}>
                                            {c.content.replace("[INTERNO] ", "") !== "[FOTO ANEXADA]" && c.content.replace("[INTERNO] ", "")}
                                            {c.proofImage && (
                        ...
                        <div className="p-4 bg-background dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800">
                            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide mb-2">
                                {macros.map((m, i) => (
                                    <button key={i} onClick={() => setNota(m)} className="whitespace-nowrap px-3 py-1.5 bg-card dark:bg-slate-900 border border-border dark:border-slate-700 rounded-full text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:border-blue-500">
                                        + {m}
                                    </button>
                                ))}
                            </div>
                            <div className="bg-card dark:bg-slate-900 rounded-2xl p-2 shadow-sm border border-border dark:border-slate-700">
...
// HELPERS (Para manter no mesmo arquivo ou mover para components/ui)
...
function ActionBtn({ icon, label, onClick, variant }: any) {
    const variants: any = {
        ...
        slate: 'bg-background dark:bg-slate-800/50 text-slate-700 dark:text-slate-400 border-border dark:border-slate-700',
    };
    return (
