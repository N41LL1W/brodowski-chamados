...
// NÃO usa useSystemConfig aqui — busca do banco diretamente
...
export default function DetalhesReadOnlyPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const [systemConfig, setSystemConfig] = useState({ systemName: 'Central de Chamados', cityName: '' });
    ...
    useEffect(() => {
        ...
        // Busca config do sistema separadamente
        fetch('/api/system-config')
            .then(r => r.json())
            .then(data => {
                if (data.system) {
                    setSystemConfig({
                        systemName: data.system.systemName || 'Central de Chamados',
                        cityName:   data.system.cityName   || '',
                    });
                }
            })
            .catch(() => {});
    ...
    const handleAssume = async () => {
        ...
        try {
            ...
            if (res.ok) router.push(`/tecnico/chamado/${id}`);
        } finally {
            setAssuming(false);
        }
    };
    ...
    if (loading) return (
        <div className="min-h-screen bg-background flex items-center justify-center">
            <div className="text-center space-y-3">
                ...
                <p className="text-muted text-[10px] font-black uppercase tracking-widest">Carregando...</p>
            </div>
        </div>
    );
        ...
        <div className="min-h-screen bg-background pb-12">
                ...
                <div className="max-w-4xl mx-auto flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                            ...
                            <h1 className="text-sm font-black text-foreground uppercase truncate">
                                {ticket.subject}
                            </h1>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                        {isDisponivel && (
                            <button onClick={handleAssume} disabled={assuming}
            ...
            <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-5">
                    ...
                    <div className="flex items-center gap-3 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-amber-600/80 dark:text-amber-500">
                                Visualizando detalhes. Clique em "Assumir chamado" para iniciar o atendimento.
                            </p>
                        </div>
                    </div>
                )}
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    <span className={`text-[10px] font-black px-4 py-1.5 rounded-full uppercase ${STATUS_STYLE[ticket.status] || 'bg-card text-muted'}`}>
                        {STATUS_LABEL[ticket.status] || ticket.status}
                    </span>
                    <span className="text-[10px] font-black px-4 py-1.5 rounded-full uppercase bg-card border border-border text-foreground">
                        {ticket.priority}
                    </span>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-5">
                    ...
                    <div className="md:col-span-2 space-y-5">
                        ...
                        <div className="bg-card border border-border rounded-3xl p-6 space-y-4">
                            <h2 className="text-[10px] font-black uppercase tracking-widest text-muted">Descrição</h2>
                            <h3 className="text-xl font-black text-foreground uppercase tracking-tight">{ticket.subject}</h3>
                            {ticket.description && (
                                <div className="p-4 bg-background rounded-2xl border border-border">
                                    <p className="text-sm text-foreground leading-relaxed italic">"{ticket.description}"</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-card border border-border rounded-3xl p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                {ticket.location && (
                                    <InfoItem icon={<MapPin size={15}/>} label="Local" value={ticket.location}/>
                                )}
                                {ticket.assignedTo && (
                                    <InfoItem icon={<Wrench size={15}/>} label="Técnico" value={ticket.assignedTo.name}/>
                                )}
                            </div>
                        </div>
                        ...
                        {ticket.location && (
                            <div className="bg-card border border-border rounded-3xl p-5">
                                ...
                                <div className="flex items-center justify-between gap-4">
                                    ...
                                    <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(ticket.location)}`}
                        ...
                        {/* VISITA */}
                        {ticket.visitDate && (
                            <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-3xl p-5">
                                <h2 className="text-[10px] font-black uppercase tracking-widest text-blue-600 dark:text-blue-400 mb-3">
                                    Visita agendada
                                </h2>
                                <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-4">
                        ...
                        <PDFDownloadLink
                            document={
                                <TicketPDF
                                    ...
                                    cityName={systemConfig.cityName}
                                />
                            }
                        ...
                        >
                            {({ loading: pdfLoading }) => (
                                <><Download size={15}/>{pdfLoading ? 'Gerando...' : 'Baixar PDF'}</>
                            )}
                        </PDFDownloadLink>
                        ...
                        <div className="bg-card border border-border rounded-3xl overflow-hidden">
                            <div className="p-4 border-b border-border flex items-center gap-2">
                                ...
                                <p className="text-[10px] font-black uppercase tracking-widest text-muted">
                                    Histórico ({ticket.comments?.filter((c: any) => !c.content.startsWith('[VISITA]') && !c.content.includes('[INTERNO]')).length || 0})
                                </p>
                            </div>
                            <div className="max-h-72 overflow-y-auto">
                                ...
                                {ticket.comments
                                    ?.filter((c: any) => !c.content.startsWith('[VISITA]') && !c.content.includes('[INTERNO]'))
                                    .map((c: any) => (
...
function InfoItem({ icon, label, value }: any) {
    ...
    return (
        <div className="flex items-start gap-3 p-3 bg-background rounded-2xl border border-border">
            <div className="p-2 bg-card rounded-xl text-muted shrink-0">{icon}</div>
            <div className="min-w-0">
