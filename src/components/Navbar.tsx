...
import { 
  ...
  Terminal, CalendarClock, UserCircle2
} from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 w-full z-50 backdrop-blur-md border-b border-border bg-card/80">
      ...
      <nav className="md:hidden flex items-center justify-around border-t border-border py-3 bg-card">
        ...
        {isLogged && ["TECNICO", "ADMIN", "MASTER"].includes(role) && (
          <MobileNavLink href="/tecnico/agenda" icon={<CalendarClock size={20}/>} active={pathname === "/tecnico/agenda"} />
        )}
...
function MobileNavLink({ href, icon, active }: any) {
    return (
        <Link href={href} className={`${active ? 'text-blue-600' : 'text-muted'}`}>
            {icon}
        </Link>
    );
}